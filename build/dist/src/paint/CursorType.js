/**
 * @Author: yanxinaliang (rainyxlxl@163.com)
 * @Date: 2018/7/5 17:16
 * @Last Modified by: yanxinaliang (rainyxlxl@163.com)
 * * @Last Modified time: 2018/7/5 17:16
 * @disc:光标、画笔类型  系统光标使用同一套方案实现，否则系统光标不支持url及光标点设置
 */
var _a;
export var CursorTypeName;
(function (CursorTypeName) {
    CursorTypeName["Pan0"] = "pan0";
    CursorTypeName["Pan1"] = "pan1";
    CursorTypeName["Pan2"] = "pan2";
    CursorTypeName["Pan3"] = "pan3";
    CursorTypeName["Pan4"] = "pan4";
})(CursorTypeName || (CursorTypeName = {}));
export var CursorType = (_a = {},
    _a[CursorTypeName.Pan0] = {
        svg: require("./svg/cursor_0.svg"),
        startPoint: {
            x: 0,
            y: 100
        }
    },
    _a);
//# sourceMappingURL=CursorType.js.map