...
export class FlowLayout<G extends fabric.Group, T extends fabric.Object, O extends IFlowLayoutOptions<T>> extends AbstractLayout<G, T, O> {
  ...
  public removeByIndex(index: number): IComponent<T>|null {
   ...
   * @param index 
   */
  valueOf(index?: number): IComponent<T>|null {
    return this._get(index as number);
  }
  ...
  protected _get(index: number): IComponent<T>|null {
      if (index > -1 && index < this.options.elements.length) {
  ...
  layout(flushCache?: boolean): void {
    ...
    let leftEdgeMargin: number = util.max([this.options.marginWidth || 0, this.options.marginLeft || 0], ""),
        rightEdgeMargin: number = util.max([this.options.marginWidth || 0, this.options.marginRight || 0], ""),
        topEdgeMargin: number = util.max([this.options.marginHeight || 0, this.options.marginTop || 0], ""),
        bottomEdgeMargin: number = util.max([this.options.marginHeight || 0, this.options.marginBottom || 0], ""),
        width: number = 0,
        height: number = 0;
