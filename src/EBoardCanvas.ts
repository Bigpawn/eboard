...
export class EBoardCanvas extends FabricCanvas implements fabric.ICanvasOptions {
     ...
     */
    constructor(
        element: HTMLCanvasElement | string, options?: IEBoardCanvasOptions) {
    ...
    protected _createCursorCanvas() {
        let lowerCanvasClass = this.getElement().
            className.
            replace(/\s*lower-canvas\s*/, '');
        if (this.cursorCanvasEl) {
            this.cursorCanvasEl.className = '';
        } else {
            this.cursorCanvasEl = this._createCanvasElement();
        }
        fabric.util.addClass(this.cursorCanvasEl, 'cursor-canvas ' +
            lowerCanvasClass);
    ...
    public addEventListener(
        eventType: FabricObservingEventType,
        listener: (event: any) => void): EBoardCanvas {
    ...
    public enableZooming() {
        ...
        this.addEventListener(FabricObservingEventType.MOUSE_WHEEL,
            this.__handleZooming);
    }
    
    public disableZooming() {
        this.removeEventListener(FabricObservingEventType.MOUSE_WHEEL,
            this.__handleZooming);
    ...
    private __handleZooming(opt: any) {
        ...
        const enablePanning = this.isEnabledPanning();
        ...
        if (enablePanning) {
            ...
            this.zoomToPoint({x: pointer.x, y: pointer.y} as fabric.Point,
                zoom);
        } else {
            this.setZoom(zoom);
        }
    ...
    public zoomToPoint(point: fabric.Point, value: number): this {
        ...
        point = fabric.util.transformPoint(point,
        ...
        this.trigger(FabricObservingEventType.ZOOM_AFTER, {
            'oldVpt': oldVpt,
            'value': value,
            'point': point,
